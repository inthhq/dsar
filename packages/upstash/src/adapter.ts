...
/**
 * Creates an Upstash Redis-backed fixed-window rate-limit store.
 *
 * @param config - Upstash client and optional key namespace.
 * @returns Store compatible with `RuntimeRateLimitConfig.store`.
 */
export const makeUpstashRateLimitStore = (
	...
	consume: async (input) => {
		const result = await config.client.eval(
			...
			[input.limit, input.windowMs, input.nowMs]
		);
		return parseUpstashEvalResult(result, input);
	},
});

export { FIXED_WINDOW_SCRIPT as upstashRateLimitFixedWindowScript };
