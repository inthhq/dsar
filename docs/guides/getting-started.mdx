...
The quickest HTTP surface is kitchen-sink, which is the local self-hosted
backend. The subject portal and dashboard are UIs that call it. For
production, point `@dsar/react` at a hosted project on inth.com.

```sh
bunx turbo run dev --filter=./examples/kitchen-sink --filter=./examples/subject-portal --filter=./examples/dashboard
```
