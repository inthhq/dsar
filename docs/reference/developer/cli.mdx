...
- `webhooks list|replay|replay-all|tail|dlq ...` -> outbound webhook dispatch inspection, replay, polling, and dead-letter recovery
...
- `dsar webhooks dlq replay --idempotency-key replay-dead-all-1`
...
`dlq list` calls `GET /webhooks/dispatches?status=dead`. `dlq replay` without
an id sends `{ "status": "dead" }` to bulk replay. `dlq replay <id>` uses the
single-dispatch replay route.
