...
This endpoint is protected and reserved for operator or service principals. It
selects `channel="webhook"` attempts in the caller's tenant. An empty body
replays `status="failed"`. Pass `status="dead"` to drain exhausted jobs.
...
- `status` (string, optional): `"failed"` or `"dead"` when present. Defaults to
  `"failed"`
- `endpoint_id` (string, optional): configured outbound webhook endpoint id
- `created_after` (string, optional): only include attempts created after this
  timestamp
...
`{ "status": "dead" }` replays exhausted dead-letter jobs instead.
