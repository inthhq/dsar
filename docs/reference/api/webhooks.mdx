...
Use `x-idempotency-key` to make a replay request safe to retry. When the same
dispatch id and idempotency key have already been accepted, the endpoint returns
...
This endpoint intentionally performs single-dispatch replay only. Bulk replay
and live dispatch tailing are deferred so operators can first validate the
smaller recovery flow safely.
