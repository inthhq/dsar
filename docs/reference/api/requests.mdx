...
- `intakeSource.type` (string, optional)
- `intakeSource.rawContextRef` (string, optional)
...
- `subject.email` (string, optional)
- `subject.externalRef` (string, optional)
- `subject.subjectId` (string, optional)
...
Create and capture decode this body with `IntakePayloadSchema`. Invalid JSON
returns `REQUEST_BODY_INVALID_JSON`. Schema failures return
`REQUEST_VALIDATION_FAILED`. Tenant and actor identity come from request
context, not from the body.
...
extensions, pauses, and segmented legal-clock history. In-process recompute
p95 on a 512-event timeline is gated by `bun run bench`. See
[Performance Benches](../testing/perf-benches.md).
